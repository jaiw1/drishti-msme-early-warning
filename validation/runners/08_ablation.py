...
-------------------------
Refit with each family in src/rigor.py GROUPS removed in turn and record
the change in grouped holdout AUC, with CIs. Two registered readings: the
cash-flow family's drop, and the largest AUC GAIN produced by dropping any
family (which must not exceed zero).
...
def run(criteria: list[Criterion], ctx: RunnerContext) -> list[Result]:
    ...
    raise NotImplementedError(
        "runner 08 pending: needs data/msme_loan_panel.csv (all feature columns "
        "grouped per src/rigor.py GROUPS)"
    )
