...
-------------------------
PSI between the training-window score distribution and the most recent
window (10 quantile bins fixed on the training window). CSI per input
feature on the same bins; report the full table and the maximum.
...
def run(criteria: list[Criterion], ctx: RunnerContext) -> list[Result]:
    ...
    raise NotImplementedError(
        "runner 06 pending: needs data/msme_loan_panel.csv (all model input "
        "columns, month_idx), app/public/demo_data.json (accounts[].pd)"
    )
