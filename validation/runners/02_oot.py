...
-------------------------
Train on early `month_idx`, test on later, embargoing training rows whose
12-month label window would cross the cut. Report the OOT AUC, the grouped
holdout AUC from runner 01, and their ratio.
...
def run(criteria: list[Criterion], ctx: RunnerContext) -> list[Result]:
    ...
    raise NotImplementedError(
        "runner 02 pending: needs data/msme_loan_panel.csv (month_idx, "
        "default_within_12m, labelable), data/rigor.json (oot.auc_early, "
        "oot.auc_late)"
    )
