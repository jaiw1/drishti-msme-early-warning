...
export default function RealModel({ data, syntheticAuc }) {
  if (!data) return <div className="text-slate-400 text-sm">Loading real-data validation…</div>
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="bg-idbi-green/5 border border-idbi-green/30 rounded-xl p-5">
        ...
        <p className="text-xs text-slate-400 mt-2">Source: {meta.source}. Predicts default within {meta.horizon_years} years from annual financials; no company appears in both training and test.</p>
      </section>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <section className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <p className="text-xs text-slate-400 mt-1 mb-4">Most important financial signals the model learned from real Indian MSMEs.</p>
        ...
        <section className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          <p className="text-xs text-slate-400 mt-1 mb-4">Predicted vs actual default rate by risk band — on the dashed line = trustworthy probabilities.</p>
          <ResponsiveContainer width="100%" height={260}>
      ...
      <section className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <span className="text-xs text-slate-400">(anonymised — real financials &amp; outcomes)</span>
        </div>
              ...
              {data.examples.map((e) => {
                ...
                return (
                  <tr key={e.id} className="border-t border-slate-50">
                    <td className="px-4 py-2.5 font-semibold text-slate-800">{e.id}<span className="text-slate-400 font-normal"> · {e.obs_year}</span></td>
                    ...
                    <td className={`px-4 py-2.5 text-right font-bold ${e.pd >= 0.5 ? 'text-rag-red' : e.pd >= 0.15 ? 'text-rag-amber' : 'text-rag-green'}`}>{Math.round(e.pd * 100)}%</td>
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${bad ? 'bg-red-50 text-rag-red' : 'bg-green-50 text-rag-green'}`}>
