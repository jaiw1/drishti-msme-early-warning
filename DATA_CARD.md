...
**Version:** SD-D6 / SD-D7 (plan §B/L4), 2026-09-16 · merges SD-D1 (`0914ac6`-era vectorisation),
SD-D2/SD-D3 (eight-portfolio population + shared latent stress), SD-D4/SD-D5 (noise, hard
negatives, labels), and the DM-2/DM-6/DM-7 export/rigor pass
**Status:** the panel this card describes is regenerated fresh by this lane, twice, from the
frozen generator on `main` @ HEAD — **45,000 accounts × 48 months** (the validation population)
and **9,000 accounts × 36 months** (the shipped demo-default population), both seed `20260709`,
both committed to `.gitignore` and reproducible with the one command each carries below. This
lane does **not** edit `src/generator/**`; every finding below is read off the data, not fixed in
it.
...
hand-edited, so it cannot drift from the file it describes. 327 nodes, grouped the way
...
| `shared.transient.arrears_ladder` | `[0.0, 22.0, 48.0, 71.0, 84.0]` | assumed | Days past due by month inside an episode that goes into arrears. The account cures the month the episode ends — which is what makes these rows hard negatives rather than mislabelled positives. Capped below 90 so a cured … | — |
...
327 parameter nodes in `src/generator/sources.yaml`, by confidence (`python3 src/data_card_params.py --census`):
...
**Result, seed 20260709, 2026-09-16:**

| Population | Checks | Pass | Fail |
|---|---|---|---|
| 45,000 × 48 (validation) | 75 | 72 | **3** |
| 9,000 × 36 (shipped demo) | 75 | 73 | **2** |

The three (two at 9k) failures are genuine, independently-discovered findings, not audit bugs —
each is carried into "Known unrealisms" below (items 13-15) rather than quietly waved through:

1. **`cc_utilisation_mode` FAILS at both sizes.** MSME-CC's credit-limit utilisation centres on a
   mode of ~0.43-0.52 (mean ~0.52-0.53), not the ~0.85 a healthy, actively-drawn cash-credit
   account conventionally runs at. `portfolios.msme_cc` carries no `base_util_mean` override in
   `src/generator/portfolios.py` (only `msme_tl` at 0.35 and `agri` at 0.68 do), so it runs on the
   shared default of 0.52.
2. **`monotone_default_by_dpd_band` FAILS at both sizes.** P(default within 12m), by the account's
   *current* DPD bucket: `0`→2.6-2.9%, `1-30`→19.7-19.3%, `31-60`→**56.6-60.7%**,
   `61-90`→**49.0-55.2%** — a reversal at the top end. The 61-90 (SMA-2) bucket is diluted by the
   hard-negative population: `arrears.arrears_ladder` lets a transient episode reach up to 84 DPD
   and then cure, so a meaningful share of the account-months sitting at 61-90 DPD are NOT
   deteriorating toward NPA. The two cleaner bands (`monotone_default_by_utilisation_band`,
   `monotone_default_by_bounce_band`) are both strictly monotone at both sizes — the reversal is
   specific to raw DPD, not a general labelling problem.
3. **`impossible_dpd_exceeds_days_on_book` FAILS at 45k only** (17 of 2,039,678 rows, 13 distinct
   accounts; does not reliably surface in a 9,000-account sample). 12 of those 17 rows are an
   account already showing a nonzero DPD (up to 30.1) in its very first observed month
   (`vintage_months == 0`) — an account cannot be days past due on an instalment before its first
   one is even due; the other 5 (4 accounts at `vintage_months == 1`, 1 at `== 2`) show a DPD that
   has climbed faster than the days the account has existed could support. A small, previously
   unflagged generator edge case, found independently by this audit.

Every other check — all eight portfolios' default-rate bands, the book-level DR-03 band, the
SMA-2/NPA ratio band, the silent-defaulter and transient-stress shares (both within 0.7pp of their
`sources.yaml` targets), every MAR-missingness rule, every channel-presence rule (14 columns × the
portfolios that are allowed to carry them), referential integrity between the two CSVs, seed
reproducibility, the vintage-hazard maturation window, the Agri-only per-account seasonal
autocorrelation, and the remaining ~30 impossible-state guards — **passes clean at both sizes.**
...
## DR-12: literal vs. CI-aware — an open ruling, not resolved here
...
**This needs the coordinator's ruling, not this lane's** (`src/realism.py`/`DATA_CARD.md` do not
touch `validation/**`). The honest framing for the README/deck either way: DR-11 (strict band
monotonicity — Green < Amber < Red) passes cleanly in all eight portfolios at 45k, which is the
property a relationship manager actually reads off the product; DR-12 is a finer-grained
diagnostic, and which of its two readings gates the go/no-go decision is a call this data card
...
13. **CC utilisation runs at roughly half-drawn, not the ~0.85 a healthy revolving limit
    conventionally shows.** `portfolios.msme_cc` carries no `base_util_mean` override, so it
    inherits the shared 0.52 default unmodified — the same starting point `msme_tl` (0.35, a term
    loan's outstanding-to-sanction ratio) and `agri` (0.68) both override away from for their own
    reasons, but MSME-CC, the portfolio where a HIGH working-capital utilisation is actually the
    healthy-and-expected reading, never does.
14. **Raw current-month DPD is not a monotone risk signal at its own top end.** The 31-60 DPD
    bucket carries a HIGHER forward 12-month default rate than the 61-90 bucket, because the
    hard-negative population's arrears ladder reaches up to 84 DPD and cures. A DPD-only baseline
    rule (the bottom rung of the DR-26 baseline ladder) would be misled by this at the deciles that
    matter most.
15. **A handful of brand-new accounts are already delinquent in their first observed month, or
    close to it.** 17 of 2,039,678 rows (13 distinct accounts) show a DPD the account's own age
    cannot support — 12 of them a nonzero DPD at `vintage_months == 0` itself. An impossible state
    (an instalment cannot already be late before the loan's first one falls due), small enough to
    not move any headline number but real and previously unflagged.
...
16. **DR-15's leakage gate (DPD family ≤ 5% attribution at 10-12 months) turns entirely on where
    ...
    item 1.
17. **DR-12's literal-vs-CI-aware split** (see the dedicated section above) is an open ruling, not
...
3. **"Why does the DPD signal reverse at 61-90 days?"** — item 14. The honest answer is that it is
   a genuine consequence of a deliberate design choice (curing hard negatives reaching up to 84
   DPD), not a bug, but it does mean "how late is the account today" cannot be read as a monotone
   risk ladder on its own, only in combination with the trajectory and the other channels.
4. **"Why is CC utilisation only ~0.52, and does that matter?"** — item 13. It does not change
   `default_within_12m`'s truth (utilisation still rises with stress, relative to each account's
   own baseline — `monotone_default_by_utilisation_band` passes cleanly), but an absolute-threshold
   rule ("flag anyone over 90% utilisation") calibrated to a real book would fire far too rarely on
   this one.
...
- **The 17-row `dpd_exceeds_days_on_book` bug (item 15) is a `src/generator/**` fix**, most likely
  in `latent.py`'s onset/severity draw for `silent_default` accounts at very low `vintage_months_0`
  — a small edge case, but a genuine one, for whichever lane next touches the generator.
- **DR-18 (cash-flow-family ablation ≥ 0.04 AUC) was only quick-diagnosed at 0.019 on a 7k sample**
  ...
  from the validation runner (L8), not this lane's data-only pass.
- **DR-12's literal-vs-CI-aware ruling** (see the dedicated section) is unresolved and belongs to
  the coordinator, not this lane.
- **Two README numbers drifted this week and are stale** (93%→89% flagged-≥6-months-ahead; the
  ...
  lane's staging scope (`src/realism.py`, `src/data_card_params.py`, `tests/test_realism.py`,
  `DATA_CARD.md` only); belongs to SB-1's stale-string grep.
...
- **`generate_data.py`'s own module docstring is stale** ("~2.7%/year slippage" — the shipped band
  is 3-5%), flagged by SD-D1's report and left for SB-1's list rather than fixed here.
